...
const convertToGeminiFormat = (messages: Message[]) => {
...
const convertJsonSchemaToGeminiSchema = (jsonSchema: Record<string, any>): any => {
  ...
  const convert = (schema: any): any => {
    ...
    if (schema.type) {
      result.type = convertType(schema.type);
    }
...
const convertGeminiResponse = (geminiResponse: any, startTime: number): InvokeResult => {
...
async function invokeGemini(params: InvokeParams): Promise<InvokeResult> {
  ...
  if (normalizedResponseFormat?.type === "json_schema") {
    const cleanedSchema = cleanSchemaForGemini(normalizedResponseFormat.json_schema.schema);
    const geminiSchema = convertJsonSchemaToGeminiSchema(cleanedSchema);
  ...
  try {
    ...
    return convertGeminiResponse({
      candidates: response.candidates,
      usageMetadata: response.usageMetadata,
    }, startTime);
  } catch (error: any) {
    throw new Error(
      `LLM invoke failed: ${error.message || error.toString()}`
    );
  }
}
...
async function invokeGroq(params: InvokeParams): Promise<InvokeResult> {
  ...
  // Groq supports json_object but not json_schema, convert if needed
  if (normalizedResponseFormat) {
    if (normalizedResponseFormat.type === "json_schema") {
